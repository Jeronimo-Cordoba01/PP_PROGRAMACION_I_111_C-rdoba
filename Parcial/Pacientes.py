    ...
    def modificar_paciente(self):
        ...
        for paciente in self.lista_pacientes:
            if paciente["dni"] == dni:
                ...
                print("Grupo sanguíneo:", paciente["grupo_sanguineo"])
        ...
        for paciente in self.lista_pacientes:
            if paciente["dni"] == dni:
                ...
                if confirmacion:
                    ...
                    self.eliminar_JSON(paciente["ID"]) 
                    break
                else:
...
#5. Ordenar pacientes. Ofrecer la opción de ordenar y mostrar la lista de pacientes de forma ascendente o descendente por: 
    def Ordenar(self):
        """
        Propósito:
            Proporcionar una funcionalidad para ordenar la lista de pacientes según diversos criterios (nombre, apellido, altura, grupo sanguíneo) en orden ascendente o descendente, basada en la entrada del usuario.
        Argumentos:
            self: Referencia a la instancia de la clase que contiene la lista de pacientes y métodos auxiliares.
        Valor de Retorno:
        ...
        def bubble_sort(pacientes, key, ascendente=True):
            ...
            for i in range(n - 1):
                for j in range(n - i - 1):
                    if ascendente:
                        if pacientes[j][key] > pacientes[j + 1][key]:
                            orden = pacientes[j]
        ...
        if asc:
            ...
            print("No se seleccionó ni ascendente ni descendente.")
            return
...
#Dar de alta en JSON
    def escribir_JSON(self, archivo: str):
        """
        Propósito:
        ...
        try:
            with open(archivo, "w") as file:
                json.dump(self.lista_pacientes, file, indent=4)
